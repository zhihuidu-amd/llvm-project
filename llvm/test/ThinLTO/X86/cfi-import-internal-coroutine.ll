; Verify that when ThinLTOBitcodeWriter creates a hidden external alias for a
; promoted internal coroutine function (replacing the self-reference in
; @llvm.coro.id with the alias), importing that coroutine into another module
; materializes the unimported alias as a Function declaration rather than a
; GlobalVariable so that @llvm.coro.id passes module verification.
